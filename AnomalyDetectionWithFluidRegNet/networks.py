...
class FluidRegNet(nn.Module):
    ...
    def forward(self, moving, fixed, diff):
        ...
        u3 = self.dec_def1(output6)
        u2 = self.dec_def2(output7)
        u1 = self.dec_def3(output8)

        a3 = self.dec_app1(output6)
        a2 = self.dec_app2(output7)
        a1 = self.dec_app3(output8)

        return u1, u2, u3, a1, a2, a3


class FluidRegNet_OneResolutionLevel(nn.Module):
    def __init__(self, n_feat):
        super().__init__()
        self.n_feat = n_feat
