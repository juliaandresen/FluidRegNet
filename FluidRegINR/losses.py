...
class HyperElasticRegularizer(torch.nn.Module):
    def __init__(self):
        super(HyperElasticRegularizer, self).__init__()

    def forward(self, input_coords, output, batch_size=None, alpha_l=1, alpha_a=1, alpha_v=1):
        grad_u = self.compute_jacobian_matrix(input_coords, output, add_identity=False)
        grad_y = self.compute_jacobian_matrix(
    ...
    def gradient(self, input_coords, output):
...
class BendingEnergyRegularizer(torch.nn.Module):

    def __init__(self):
        super(BendingEnergyRegularizer, self).__init__()

    def forward(self, input_coords, output, batch_size):
        """Compute the bending energy."""

        jacobian_matrix = self.compute_jacobian_matrix(input_coords, output, add_identity=False)
        ...
        for i in range(3):
            dx_xyz[:, i, :] = self.gradient(input_coords, jacobian_matrix[:, i, 0])
            dy_xyz[:, i, :] = self.gradient(input_coords, jacobian_matrix[:, i, 1])
            dz_xyz[:, i, :] = self.gradient(input_coords, jacobian_matrix[:, i, 2])
...
class ExclusionLossCoords_corr(torch.nn.Module):
    '''
    ...
    def forward(self, input_coords, output_1, output_2):
        ...
        jac_1 = self.compute_jacobian_matrix(input_coords, output_1)
        jac_2 = self.compute_jacobian_matrix(input_coords, output_2)
